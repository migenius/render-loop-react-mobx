...
export default class RSCamera {
     ...
     */
    @action
    pan(horizontal, vertical, shiftTargetPoint=true) {
     ...
     */
    @action
    dolly(depth, shiftTargetPoint=false) {
     ...
     */
    @action
    orbitAroundPoint(point, verticalAxis, horizontalAxis, shiftTargetPoint=false) {
     ...
     */
    @action
    rotate(verticalAxis, horizontalAxis, directionAxis, shiftTargetPoint=true) {
     ...
     */
    @action
    tilt(horizontalAxis, shiftTargetPoint=true) {
     ...
     */
    @action
    spin(verticalAxis, shiftTargetPoint=true) {
     ...
     */
    @action
    rotateAroundAxis(axis, angle, inCameraSpace=false, shiftTargetPoint=true) {
     ...
     * @param shiftTargetPoint Set this parameter to true to shift the target point of the camera along the vector
     * new_position -> old_position.
     */
    @action
    moveTo(location, shiftTargetPoint=true) {
     ...
     */
    @action
    translate(move, shiftTargetPoint=true) {
    ...
    transformPoint(point, result) {
        let world2Cam = this.transform.worldToObj;
        result.set(point);
    ...
    transformDirectionToWorld(direction, result) {
        let cam2World = this.transform.worldToObj.clone();
        let myDir = direction.clone();
        cam2World.invert();
    ...
    equal(rhs) {
        ...
        if (this.aperture === rhs.aperture &&
            ...
            let matrix1 = this.transform.worldToObj;
            let matrix2 = rhs.transform.worldToObj;
    ...
    levelCamera() {
        ...
        let angle = Math.asin(this.direction.dot(this.m_transform.getDefaultUpDirection()));
    ...
    */
     ...
     */
    @action
    setTargetPoint(targetPoint, resetUpVector=true) {
    ...
    @action
    setLocation(loc, moveTargetPoint=false) {
    ...
    set followTargetPoint(follow) {
        let oldValue = this.transform.getFollowTargetPoint();
        if (oldValue !== follow) {

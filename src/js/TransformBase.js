...
export default class TransformBase {
     ...
     */
    clone() {
        let transform = new TransformBase();
    ...
    deriveVectors(worldToObj) {
        let obj_to_world = new Matrix4x4(worldToObj);
        obj_to_world.invert();
     ...
     */
    _rotateVectors(axis, angle, rotationVectors, inObjectSpace=false) {
        let m = new Matrix4x4();
        if (!!inObjectSpace) {
        ...
        for (let i = 0; i < rotationVectors.length; i++) {
     ...
     */
    @action
    _rotateXVectors(axis, angle, rotationVectors, inObjectSpace=false) {
     ...
     */
    @action
    _rotateZVectors(axis, angle, rotationVectors, inObjectSpace=false) {
    ...
    _lookAtPoint(point, up, setTranslation) {
        ...
        let to_point = point.clone().subtract(this.m_translation);
        to_point.normalize();

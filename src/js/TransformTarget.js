...
export default class TransformTarget extends TransformBase {
    ...
    @computed
    get worldToObj() {
        return this.deriveWorldToObj(
            this.m_x_axis,this.m_y_axis,this.m_z_axis.clone().scale(-1),
            this.m_translation,
            this.m_scale);
    }
     ...
     */
    @action
    setTargetPoint(targetPoint, resetYVector=true) {
     ...
     */
    @action
    translateTargetPoint(dx, dy, dz, inObjectSpace=true) {
     ...
     */
    @action
    translate(dx, dy, dz, inObjectSpace=true, translateTarget=true) {
     ...
     */
    @action
    setTranslation(x, y, z, translateTarget=true) {
    ...
    @action
    _rotateYVectors(axis, angle, rotationVectors, inObjectSpace=false) {
     ...
     */
    @action
    rotate(dx, dy, dz, rotateTargetPoint=true) {
     ...
     */
    @action
    setRotation(x, y, z, rotateTargetPoint=true) {
        if (rotateTargetPoint || !this.m_follow_target_point) {
     ...
     * then the axis will be transform into object space first. If rotateTargetPoint is set to true then the target
     * point is also rotated.
     */
    @action
    rotateAroundAxis(axis, angle, inObjectSpace=true, rotateTargetPoint=true) {
    ...
    /**
     * Sets the rotation of the transform about the given axis by the given angle in radians. If inObjectSpace is set
     * to true, then the axis will be transform into object space first. If rotateTargetPoint is set to true then the
     * target point is also rotated.
     */
    @action
    setRotationAroundAxis(axis, angle, rotateTargetPoint=true) {
        this.m_x_axis.set(TransformBase.X_AXIS);
        this.m_y_axis.set(TransformBase.Y_AXIS);
        this.m_z_axis.set(TransformBase.NEG_Z_AXIS);
     ...
     */
    @action
    rotateAroundPoint(point, dx, dy, dz, rotateTargetPoint=true) {
